...
struct Camera {
    ...
    void update() {
        uint64_t renderPtr = mem::Read<uint64_t>(OFF_REGION + OFF_VIEW_RENDER, xorstr_("Camera renderPtr"));
        uint64_t matrixPtr = mem::Read<uint64_t>(renderPtr + OFF_VIEW_MATRIX, xorstr_("Camera matrixPtr"));
        gameViewMatrix = mem::Read<ViewMatrix>(matrixPtr, xorstr_("Camera gameViewMatrix"));
    }
