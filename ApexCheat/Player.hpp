...
struct Player {
    ...
    void readFromMemory( Level* map, LocalPlayer* lp, int counter) {
        llp = lp;
        base = mem::Read<uint64_t>(OFF_REGION + OFF_ENTITY_LIST + ((index + 1) << 5), xorstr_("Player base"));
        if (base == 0) return;
        name = mem::ReadString(base + OFF_NAME, 1024, xorstr_("Player name"));
        isPlayer = name == "player";
        isDrone = name == "drone_no_minimap_object";
        if (map->isTrainingArea) {
            teamNumber = mem::Read<int>(base + OFF_TEAM_NUMBER, xorstr_("Player teamNumber"));
        ...
        if (!map->isTrainingArea)
            teamNumber = mem::Read<int>(base + OFF_TEAM_NUMBER, xorstr_("Player teamNumber"));
        localOrigin = mem::Read<Vector3D>(base + OFF_LOCAL_ORIGIN, xorstr_("Player localOrigin"));
        ...
        if (isPlayer) {
            viewAngles = mem::Read<Vector2D>(base + OFF_VIEW_ANGLES, xorstr_("Player viewAngles"));
            viewYaw = mem::Read<float>(base + OFF_YAW, xorstr_("Player viewYaw"));
            plyrDataTable = mem::Read<int>(base + OFF_NAME_INDEX, xorstr_("Player plyrDataTable"));
            if (FEATURE_SPECTATORS_ON && counter % 99 == 0) {
                spectators = mem::Read<uint64_t>(OFF_REGION + OFF_OBSERVER_LIST, xorstr_("Player spectators"));
                spctrIndex = mem::Read<int>(spectators + plyrDataTable * 8 + OFF_OBSERVER_ARRAY, xorstr_("Player spctrIndex"));
                spctrBase = mem::Read<uint64_t>(OFF_REGION + OFF_ENTITY_LIST + ((spctrIndex & 0xFFFF) << 5), xorstr_("Player spctrBase"));
            }
        }
        isDead = (isDrone || isDummie) ? false : mem::Read<short>(base + OFF_LIFE_STATE, xorstr_("Player isDead")) > 0;
        isKnocked = (isDrone || isDummie) ? false : mem::Read<short>(base + OFF_BLEEDOUT_STATE, xorstr_("Player isKnocked")) > 0;
        currentHealth = mem::Read<int>(base + OFF_HEALTH, xorstr_("Player currentHealth"));
        currentShield = mem::Read<int>(base + OFF_SHIELD, xorstr_("Player currentShield"));
        ...
        lastTimeVisible = mem::Read<float>(base + OFF_LAST_VISIBLE_TIME, xorstr_("Player lastTimeVisible"));
    ...
    std::string getPlayerName() {
        uintptr_t nameOffset = mem::Read<uintptr_t>(OFF_REGION + OFF_NAME_LIST + ((plyrDataTable - 1) * 24), xorstr_("Player nameOffset"));
        std::string playerName = mem::ReadString(nameOffset, 64, xorstr_("Player playerName"));
        return playerName;
    }

    int GetPlayerLevel() {
        int m_xp = mem::Read<int>(base + OFF_XP_LEVEL, xorstr_("Player m_xp"));
        if (m_xp < 0) return 0;
    ...
    int getBoneFromHitbox(HitboxType hitbox) const {
        uint64_t modelPointer = mem::Read<uint64_t>(base + OFF_STUDIO_HDR, xorstr_("Player modelPointer"));
        if (!mem::IsValidPointer(modelPointer))
            return -1;

        uint64_t studioHdr = mem::Read<uint64_t>(modelPointer + 0x8, xorstr_("Player studioHdr"));
        if (!mem::IsValidPointer(studioHdr + 0x34))
            return -1;

        auto hitboxCache = mem::Read<uint16_t>(studioHdr + 0x34, xorstr_("Player hitboxCache"));
        auto hitboxArray = studioHdr + ((uint16_t)(hitboxCache & 0xFFFE) << (4 * (hitboxCache & 1)));
        if (!mem::IsValidPointer(hitboxArray + 0x4))
            return -1;

        auto indexCache = mem::Read<uint16_t>(hitboxArray + 0x4, xorstr_("Player indexCache"));
        ...
        return mem::Read<uint16_t>(bonePointer, xorstr_("Player bonePointer"));
    }
    ...
    Vector3D getBonePosition(HitboxType hitbox) const {
        ...
        uint64_t boneMatrixPtr = mem::Read<uint64_t>(base + OFF_BONE, xorstr_("Player boneMatrixPtr"));
        boneMatrixPtr += (bone * sizeof(Matrix3x4));
        if (!mem::IsValidPointer(boneMatrixPtr))
            return localOrigin.Add(offset);

        Matrix3x4 boneMatrix = mem::Read<Matrix3x4>(boneMatrixPtr, xorstr_("Player boneMatrix"));
    ...
    {
        uint64_t ptrInt = mem::Read<uint64_t>(base + OFF_GLOW_THROUGH_WALL, xorstr_("Player GlowThroughWall"));
    ...
    int getGlowEnable()
    {
        uint64_t ptrInt = mem::Read<uint64_t>(base + OFF_GLOW_HIGHLIGHT_ID, xorstr_("Player GlowEnable"));
        if (!mem::is_valid(ptrInt))
            return -1;
        return ptrInt;
    }
