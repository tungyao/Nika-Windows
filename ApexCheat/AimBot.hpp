...
struct AimBot {
	...
	AimBot(MyDisplay* in_myDisplay, LocalPlayer* in_lp, std::vector<Player*>* in_players, Camera* in_gameCamera) {
	...
	bool active(bool leftLock, bool rightLock) {
		bool isAimbotOn = FEATURE_AIMBOT_ON;
	...
	bool isValidTarget(Player* target) {
		if (target == nullptr
			...
			|| target->distance2DToLocalPlayer < util::metersToInches(AIMBOT_MIN_DISTANCE))
			return false;
		return true;
	}
	...
	void getAnglesToTarget(Player* target, Vector3D targetPosition, Vector2D& angles) {
		if (AIMBOT_PREDICT_BULLETDROP && lp->weaponProjectileSpeed > 999.9f)
		...
		if (AIMBOT_PREDICT_MOVEMENT && lp->weaponProjectileSpeed > 999.9f)
	...
	void update(bool leftLock, bool rightLock, bool autoFire, int boneId, int totalSpectators) {
		...
		if (lp->inZoom) {
			finalFov = AIMBOT_FOV * lp->zoomFov / 60;
			finalFov += finalFov * (60 / lp->zoomFov - 1) * AIMBOT_FOV_EXTRA_BY_ZOOM;
			finalDistance = AIMBOT_MAX_DISTANCE;
		}
		else {
			finalFov = (AIMBOT_FOV + 20);
			finalDistance = TRIGGERBOT_HIPFIRE_RANGE;
		}

		Player* target = currentTarget;
		if (!isValidTarget(target)) {
			if (targetSelected && !AIMBOT_ALLOW_TARGET_SWITCH)
				return;
		...
		int zoomedMaxMove = AIMBOT_ZOOMED_MAX_MOVE;
		int hipfireMaxMove = AIMBOT_HIPFIRE_MAX_MOVE;
		int maxDelta = AIMBOT_MAX_DELTA;
		if (distanceFromCrosshair < finalFov * AIMBOT_FAST_AREA) {
			zoomedMaxMove *= AIMBOT_FAST_AREA;
			hipfireMaxMove *= AIMBOT_FAST_AREA;
			maxDelta /= AIMBOT_WEAKEN;
		}
		else if (distanceFromCrosshair < finalFov * AIMBOT_SLOW_AREA) {
			zoomedMaxMove *= AIMBOT_SLOW_AREA;
			hipfireMaxMove *= AIMBOT_SLOW_AREA;
			maxDelta /= AIMBOT_WEAKEN;
		}
		...
		float totalSmooth;
		if (AIMBOT_LEGACY_MODE)
			totalSmooth = AIMBOT_SMOOTH / 10 + AIMBOT_SMOOTH_EXTRA_BY_DISTANCE / currentTarget->distanceToLocalPlayer;
		else
			totalSmooth = AIMBOT_SMOOTH * 10;
		float bulletSpeed = lp->weaponProjectileSpeed;
		if (!lp->inZoom)
			//totalSmooth *= (cl->AIMBOT_WEAKEN + 1) / 2;
			totalSmooth *= AIMBOT_WEAKEN;
		else
			if (!currentTarget->isDrone && (!leftLock || AIMBOT_SPECTATORS_WEAKEN && totalSpectators > 0)) {
				totalSmooth *= AIMBOT_WEAKEN;
				bulletSpeed /= AIMBOT_WEAKEN;
			}

		Vector2D aimbotDelta = desiredAnglesIncrement.Multiply(AIMBOT_SPEED).Divide(totalSmooth);
		...
		if (!AIMBOT_LEGACY_MODE) {
			...
			totalPitchIncrementInt = std::round((targetBoneW2S.y - screenSize.y / 2) * AIMBOT_SPEED / totalSmooth);
			totalYawIncrementInt = std::round((targetBoneW2S.x - screenSize.x / 2) * AIMBOT_SPEED / totalSmooth);
		}
		...
		myDisplay->moveMouseRelative(totalPitchIncrementInt, totalYawIncrementInt);
