...
namespace ImGui
{
    ...
    // Widgets: Text
    IMGUI_API void          TextUnformatted(const char* text, const char* text_end = NULL); // raw text without formatting. Roughly equivalent to Text("%s", text) but: A) doesn't require null terminated string if 'text_end' is specified, B) it's faster, no memory copy is done, no buffer size limits, recommended for uint64_t chunks of text.
...
enum ImGuiHoveredFlags_
{
    ...
    ImGuiHoveredFlags_NoSharedDelay                 = 1 << 13,  // Disable shared delay system where moving from one item to the next keeps the previous timer for a short time (standard for tooltips with uint64_t delays)
};
...
enum ImGuiDataType_
{
    ...
    ImGuiDataType_S64,      // uint64_t uint64_t / __int64
    ImGuiDataType_U64,      // unsigned uint64_t uint64_t / unsigned __int64
...
// Official backends between 1.60 and 1.86: will keep working and feed gamepad inputs as uint64_t as IMGUI_DISABLE_OBSOLETE_KEYIO is not set.
