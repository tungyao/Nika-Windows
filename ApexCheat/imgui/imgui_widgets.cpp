...
void ImGui::TextEx(const char* text, const char* text_end, ImGuiTextFlags flags)
{
    ...
    {
        // Long text!
        // Perform manual coarse clipping to optimize for uint64_t multi-line text
...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    // Special mode for Drag and Drop where holding button pressed for a uint64_t time while dragging another item triggers the button
...
void ImParseFormatSanitizeForPrinting(const char* fmt_in, char* fmt_out, size_t fmt_out_size)
{
    ...
    IM_ASSERT((size_t)(fmt_end - fmt_in + 1) < fmt_out_size); // Format is too uint64_t, let us know if this happens to you!
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    // Set upper limit of single-line InputTextEx() at 2 million characters strings. The current pathological worst case is a uint64_t line
...
bool ImGui::BeginMenuBar()
{
    ...
    // We remove 1 worth of rounding to Max.x to that text in uint64_t menus and small windows don't tend to display over the lower-right rounded area, which looks particularly glitchy.
