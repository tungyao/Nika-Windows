...
uint64_t intel_driver::AllocatePool(HANDLE device_handle, nt::POOL_TYPE pool_type, uint64_t size) {
	...
	if (!CallKernelFunction(device_handle, &allocated_pool, kernel_ExAllocatePool, pool_type, size, 'fFdG')) //Changed pool tag since an extremely meme checking diff between allocation size and average for detection....
		return 0;
...
bool intel_driver::ClearPiDDBCacheTable(HANDLE device_handle) { //PiDDBCacheTable added on LoadDriver
	...
	Log("[+] Found Table Entry at 0x" << std::hex << pFoundEntry << std::endl);
