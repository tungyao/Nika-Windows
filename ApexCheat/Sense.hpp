...
struct Sense {
	...
	Sense(LocalPlayer* in_lp, std::vector<Player*>* in_players, Camera* in_gameCamera, AimBot* in_aimBot) {
	...
	void renderESP(ImDrawList* canvas) {
		...
		for (int i = 0; i < players->size(); i++) {
			...
			if (!SENSE_SHOW_DEAD && !p->currentHealth > 0) continue;
			...
			if (p->isEnemy && p->isValid() && !p->isLocal && distance < SENSE_MAX_RANGE) {
				...
				if (isLocalOriginW2SValid && isHeadPositionW2SValid) {
					...
					if (SENSE_SHOW_PLAYER_DISTANCES) {
						...
						drawText(canvas, drawPosition, std::to_string((int)distance).c_str(), enemyBoxColor);
					}
					// Draw Name
					if (SENSE_SHOW_PLAYER_NAMES) {
						if (SENSE_TEXT_BOTTOM)
							drawPosition = drawPosition.Add(Vector2D(0, 20));
						else
							drawPosition = drawPosition.Subtract(Vector2D(0, 20));
						const char* txtName = "Dummie";
						if (p->isPlayer)
					...
					// Draw Level
					if (SENSE_SHOW_PLAYER_LEVELS && p->isPlayer) {
						if (SENSE_TEXT_BOTTOM)
							drawPosition = drawPosition.Add(Vector2D(0, 20));
						else
							drawPosition = drawPosition.Subtract(Vector2D(0, 20));
		...
		// Draw FOV circle
		if (SENSE_SHOW_FOV && lp->inZoom) {
			ImVec2 center(screenSize.x / 2, screenSize.y / 2);
			int radius = screenSize.y / 60 * AIMBOT_FOV;
			radius += radius * (60 / lp->zoomFov - 1) * AIMBOT_FOV_EXTRA_BY_ZOOM;
			canvas->AddCircle(center, radius, ImColor(ImVec4(1.00f, 1.00f, 1.00f, 1.00f)));
		}

		// Draw target line
		if (SENSE_SHOW_TARGET && aimBot->targetSelected) {
	...
	void update(int counter) {
		...
		for (std::size_t i = 0; i < players->size(); i++) {
			...
			double distance = math::calculateDistanceInMeters(
				...
				p->localOrigin.z);
			if (p->isValid()) {
				if (!p->isVisible && distance < SENSE_MAX_RANGE) {

					p->setGlowEnable(1);
					p->setGlowThroughWall(1);

					p->setCustomGlow(0, false, p->isFriendly);
				}
				else
					if (!p->isKnocked && distance < SENSE_MAX_RANGE) {
						p->setGlowEnable(1);
						p->setGlowThroughWall(1);
						int healthShield = p->currentHealth + p->currentShield;
						p->setCustomGlow(healthShield, true, p->isFriendly);
					}
	...
	void renderRadar(ImDrawList* canvas) {
		// 1920*1080: 215 x 215
		// 2560*1440: 335 x 335
		ImGui::SetNextWindowSize({ (float)FEATURE_MAP_RADAR_X, (float)FEATURE_MAP_RADAR_Y });
		...
		for (int i = 0; i < players->size(); i++) {
			...
			if (radarDistance >= 0.0f && radarDistance < SENSE_MAX_RANGE) {
