...
namespace Discord
{
	...
	namespace Drawing
	{
		...
		inline void EnsureSkiaInitialized(Frame& frame)
		{
			...
			// 将像素数据指定为绘图表面的像素缓冲区
			_canvas = SkCanvas::MakeRasterDirect(info, frame.Buffer, frame.Width * 4).release();
...
namespace Hjack {
	...
	{
		const string alpha_numeric(xorstr_("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz1234567890!@#$%^&*"));
